...
class List {
 public:
    ...
    ListNode<NODETYPE>* searchList(const NODETYPE&) const;
...
/**
 * Recursively search list for first instance of given value.
 * @return pointer to value or null.
 */
template <typename NODETYPE>
ListNode<NODETYPE>* List<NODETYPE>::searchList(const NODETYPE& val) const {
    static ListNode<NODETYPE> *currentPtr = begin();

    if (currentPtr == nullptr || currentPtr->getData() == val) {
        return currentPtr;
    }
